...
<body>
    <main class="main-int_LNEC main-faturas-emitidas">
        ...
        <section class="content-int_LNEC">
                    ...
                    <div class="search-result-show_LNEC">
                        <div class="content-result">
                            <table class="search-result-table_LNEC">
                                ...
                                $sql = "SELECT CODRECEI, NUMDOCUM, situacaoreceita, CAST(DATACERT AS DATE), PORRECEB FROM faturacao INNER JOIN situacao_receita ON faturacao.ESTADO = situacao_receita.codsitrec LIMIT 10 OFFSET $inicio;";
                                ...
                                if ($stmt = mysqli_prepare($link, $sql)) {
                                    ...
                                    if (mysqli_stmt_execute($stmt)) {
                                        ...
                                        if (mysqli_stmt_num_rows($stmt) >= 1) {
                                            ...
                                            mysqli_stmt_bind_result($stmt, $id, $id_f,$estado, $data, $valor);
                                            while (mysqli_stmt_fetch($stmt)){


                                                ?>
                                                <tr class="row-item_table" onclick="window.open('http://localhost/LNEC/fatura2.php?id=<?php echo $id; ?>')" >
            ...
            <?php
            $resultado = $link->query("SELECT * FROM faturacao");
            ...
            $i=1;
            while ($i <= $pagina) {
                echo'<a href="faturas-emitidas_lnec.php?pag='.$i.'" style="  padding: 5px;background-color: red;color: white;border-radius: 50%;text-decoration: none;
">'.$i.'</a>&nbsp;&nbsp;';
                $i++;
            }
            ?>
